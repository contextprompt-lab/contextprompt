...
export const testAnalysis = (transcript: string, repoMaps?: ClientRepoMap[]) => request<{ ok: true; meeting_id: number; status: string }>('/meetings/test-analysis', { method: 'POST', body: JSON.stringify({ transcript, repo_maps: repoMaps }), headers: { 'Content-Type': 'application/json' } });
